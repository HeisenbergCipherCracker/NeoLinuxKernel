...
uint32 determine_blk_group(uint32 inode){
	printf("\nUsing inode: %d", inode);
	ext2_superblock* sb = read_superblock();
    uint32 block_group = (inode - 1) / sb->inodes_per_group;
	printf("\nInodes per group: %d", sb->inodes_per_group);
    return block_group;
}
...
ext2_bgdt* parse_bgdt(uint32 block_group) {
	printf("\nGROUP: %d\n", block_group);
	ext2_superblock* sb = read_superblock();
	ext2_bgdt* bgdt = kmalloc(sizeof(*bgdt)); 
	uint32 blk_group_start_block = (block_group - 1) * sb->blocks_per_group + 1;
	printf("\nSTART BLOCK: %d", blk_group_start_block);
...
ext2_inode* read_inode(uint32 inode) {
	...
	ext2_bgdt* inode_bgdt = kmalloc(sizeof(*inode_bgdt));
	...
	uint32* inode_tmp = kmalloc(sizeof(*inode_tmp));
...
int ext2_ls(uint32 inode_num){
	...
	if(inode_num == 0){
		...
		ide_read_sectors(DRIVE, 1, 2 * datablock0, dir);
		int curr = 0;
		int next;
		int i;
		while(1){
...
uint32 ext2_path_to_inode(char* path){
	...
	while(path[i]){
		...
		// Parsing Path
			...
			printf("\nKMALLOC(%d)", i);
			char* lookup_name = kmalloc(sizeof(char*));//kcalloc(end - start, 1);
			int y = 0;
